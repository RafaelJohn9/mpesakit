...
class B2CResultMetadata(BaseModel):
    ...
    def _get_parameters_dict(self) -> dict[str, str | int | float]:
        """Cache and return ResultParameters as a dict for easy access."""
        if not hasattr(self, "_parameters_dict_cache"):
            if not self.ResultParameters:
                self._parameters_dict_cache = {}
            else:
                self._parameters_dict_cache = {
                    param.Key: param.Value for param in self.ResultParameters
                }
        return self._parameters_dict_cache

    @property
    def transaction_amount(self) -> Optional[int | float]:
        """Return the TransactionAmount value if present."""
        return self._get_parameters_dict().get("TransactionAmount")

    @property
    def transaction_receipt(self) -> Optional[str]:
        """Return the TransactionReceipt value if present."""
        return self._get_parameters_dict().get("TransactionReceipt")
    ...
    def recipient_is_registered(self) -> Optional[bool]:
        ...
        val = self._get_parameters_dict().get("B2CRecipientIsRegisteredCustomer")
        if val == "Y":
            return True
        if val == "N":
            return False
    ...
    def receiver_party_public_name(self) -> Optional[str]:
        ...
        return self._get_parameters_dict().get("ReceiverPartyPublicName")

    @property
    def transaction_completed_datetime(self) -> Optional[str]:
        """Return the TransactionCompletedDateTime value if present."""
        return self._get_parameters_dict().get("TransactionCompletedDateTime")
