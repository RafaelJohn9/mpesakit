...
def test_post_success(client):
    ...
    with patch("mpesa_sdk.http_client.mpesa_http_client.requests.post") as mock_post:
        mock_response = Mock()
...
def test_post_request_exception(client):
    ...
    with patch(
        "mpesa_sdk.http_client.mpesa_http_client.requests.post",
...
def test_get_success(client):
    ...
    with patch("mpesa_sdk.http_client.mpesa_http_client.requests.get") as mock_get:
        mock_response = Mock()
...
def test_get_request_exception(client):
    ...
    with patch(
        "mpesa_sdk.http_client.mpesa_http_client.requests.get",
        side_effect=requests.RequestException("boom"),
    ):
        with pytest.raises(MpesaApiException) as exc:
            client.get("/fail")
